...
namespace UnityEngine.XR.ARFoundation
{
    ...
    public static class ARPlaneMeshGenerator
    {
        ...
        static bool TryTriangulate()
        {
            s_CurrentUnusedVertexNode = s_UnusedVertices.Last;
            int vertexIterationsSinceLastRemoval = 0;

            while (s_UnusedVertices.Count > 2)
            {
                // Handle degenerate polygons: stop checking if we've checked all vertices without finding a valid ear.
                // This will prevent infinite loop and the application hanging.
                if (vertexIterationsSinceLastRemoval >= s_UnusedVertices.Count)
                {
                    Debug.LogError("Cannot triangulate mesh for a plane because its vertices do not form a simple polygon");
                    return false;
                }
                ...
                {
                    vertexIterationsSinceLastRemoval++;
                    continue;
                }
                ...
                vertexIterationsSinceLastRemoval = 0;
            }
